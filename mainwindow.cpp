...
void MainWindow::addInvoice() {
    QString idText = ui->lineId->text().trimmed();
    QString montantText = ui->lineMontant->text().trimmed();
    ...
    if (idText.isEmpty()) {
        erreurs << "• L'ID de la facture est obligatoire";
    }
    ...
    double montant = montantText.toDouble(&okMontant);

    if (!okId || id <= 0) {
        erreurs << "• L'ID doit être un nombre positif";
    }
    ...
    if (!emission.isValid()) {
        erreurs << "• La date d'émission est invalide";
    }

    if (!echeance.isValid()) {
        erreurs << "• La date d'échéance est invalide";
    }

    if (emission.isValid() && echeance.isValid() && echeance <= emission) {
        erreurs << "• La date d'échéance doit être après la date d'émission";
    }
    ...
    QSqlQuery q;
    q.prepare("INSERT INTO MOUTIE.FACTURE "
              "(ID_FACTURE, MONTANT, DATE_D_EMMISSION, DATE_D_ECHAENCE, IDEMPOLYE, IDSPONSOR, STATUT) "
    ...
    q.bindValue(":idemploye", 1);  // ID employé par défaut
    q.bindValue(":idsponsor", 84);  // ID sponsor valide de votre BD
    q.bindValue(":status", status);

    if (q.exec()) {
        QMessageBox::information(this, "Succès", "✅ Facture ajoutée avec succès !");
        loadInvoicesFromDatabase();
        clearForm();
    } else {
        QMessageBox::critical(this, "Erreur DB", "Erreur lors de l'ajout :\n" + q.lastError().text());
    }
}
void MainWindow::loadInvoicesFromDatabase() {
    ...
    // CORRECTION: Utiliser les bons noms de schéma, tables et colonnes
    QString sql =
        "SELECT F.ID_FACTURE, F.MONTANT, F.DATE_D_EMMISSION, F.DATE_D_ECHAENCE, "
        "       F.STATUT, E.PRENOM AS EMPLOYEE_NAME, S.NOM AS SPONSOR_NAME "
        "FROM MOUTIE.FACTURE F "
        "LEFT JOIN MOUTIE.EMPOLYE E ON F.IDEMPOLYE = E.IDEMPLOYE "
        "LEFT JOIN MOUTIE.SPONSOR S ON F.IDSPONSOR = S.IDSPONSOR "
    ...
    if (!query.exec(sql)) {
        ...
        QSqlQuery checkQuery;
        if (checkQuery.exec("SELECT COUNT(*) FROM USER_TABLES WHERE TABLE_NAME = 'FACTURE'")) {
            if (checkQuery.next() && checkQuery.value(0).toInt() == 0) {
                // Créer la table si elle n'existe pas
                QSqlQuery createQuery;
                QString createSql =
                    "CREATE TABLE MOUTIE.FACTURE ("
                    "ID_FACTURE NUMBER PRIMARY KEY, "
                    "MONTANT NUMBER, "
                    "DATE_D_EMMISSION DATE, "
                    "DATE_D_ECHAENCE DATE, "
                    "STATUT VARCHAR2(20), "
                    "IDEMPOLYE NUMBER, "
                    "IDSPONSOR NUMBER, "
                    "CONSTRAINT FACTFK FOREIGN KEY (IDEMPOLYE) REFERENCES MOUTIE.EMPOLYE(IDEMPLOYE), "
                    "CONSTRAINT FINANCFK FOREIGN KEY (IDSPONSOR) REFERENCES MOUTIE.SPONSOR(IDSPONSOR))";

                if (createQuery.exec(createSql)) {
                    qDebug() << "Table FACTURE créée avec succès";
                } else {
...
void MainWindow::editInvoice() {
    ...
    QSqlQuery q;
    // CORRECTION: utiliser STATUT
    q.prepare("UPDATE MOUTIE.FACTURE SET MONTANT = :montant, DATE_D_EMMISSION = :emission, "
              "DATE_D_ECHAENCE = :echeance, STATUT = :status WHERE ID_FACTURE = :id");
    q.bindValue(":montant", montant);
    q.bindValue(":emission", emission);
    q.bindValue(":echeance", echeance);
...
void MainWindow::deleteInvoice() {
    ...
    int id = ui->tableFactures->item(row, 0)->text().toInt();
    QSqlQuery q;
    // CORRECTION: ajouter le schéma
    q.prepare("DELETE FROM MOUTIE.FACTURE WHERE ID_FACTURE = :id");
    q.bindValue(":id", id);
